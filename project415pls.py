...
clustering = DBSCAN(eps=3, min_samples=5).fit(df[['streamerID', 'broadCasterID', 'totalViews']])
newdf = (df-df.mean())/df.std()
newdf['cluster'] = clustering.labels_
newdf['cluster'] = clustering.labels_
df[df['cluster'] != -1].to_csv('clustering.csv')
df.drop(['broadCasterName', 'GameName', 'streamCreatedTime', 'partnerStatus', 'broadcasterLanguage', 'language', 'broadcastersCreatedTime', 'SourceResolution'], axis=1, inplace=True)
print(df.shape)
print(df)
